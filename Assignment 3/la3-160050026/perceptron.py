...
class PerceptronClassifier:
    ...
    def train( self, trainingData, trainingLabels, sample_weights=None):
        ...
        for iteration in range(self.max_iterations):
            for i in range(len(trainingData)): 
                ...
                # cl = int(np.sign(self.weights * trainingData[i]))
    ...
    def sample_data(self, trainingData, trainingLabels, sample_weights):
        values = [(trainingData[i], trainingLabels[i]) for i in range(len(trainingData))]
        data = util.nSample(sample_weights, values, int(0.5*len(trainingData)))
        trainingData1 = [x for (x,y) in data]
        trainingLabels1 = [y for (x,y) in data]
        return trainingData1, trainingLabels1
