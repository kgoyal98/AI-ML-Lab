...
class BaggingClassifier:
    ...
    def train( self, trainingData, trainingLabels):
        ...
        for i in range(self.num_classifiers):
            trainingData1 = []
            trainingLabels1 = []
            for s in range(int(self.ratio*len(trainingData))):
                x = random.choice(range(len(trainingData)))
                # print(x)
                trainingData1.append(trainingData[x])
                trainingLabels1.append(trainingLabels[x])
            self.classifiers[i].train(trainingData1, trainingLabels1)
    ...
    def classify( self, data):
        ...
        """
        # print([classifier.classify(data) for classifier in self.classifiers])

        X = np.sign(sum(np.array([classifier.classify(data) for classifier in self.classifiers])))
        return [np.random.choice([-1, 1]) if x==0 else x for x in X]
