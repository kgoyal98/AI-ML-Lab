    ...
    if(n<=2):
    	...
    	for l in range(2, n-1):
    		for i in range(1, n-l+1):
    			mid = tour[i:i+l]
    			# print(tour, mid, tour[:i], tour[i+l:])
    			neighbour = tour[:i]+mid[::-1]+tour[i+l:]
    			all_possible_neighbours.append(neighbour)
    	assert(len(all_possible_neighbours)==n*(n-3)/2)
    "*** --------------  ***"
...
def generate3optNeighbours(tour):
    ...
    for i in range(1, n):
        for j in range(0,n):
            for k in range(0, n):
                if(k==0):
                    if ((j-i)>=2 and (n-j)>=2 and i>=2):
                        part1=tour[i:j]
                        part2=tour[j:]
                        all_possible_neighbours.append(tour[:i]+part1[::-1]+ part2[::-1])
                        all_possible_neighbours.append(tour[:i]+ part2+part1[::-1])
                        all_possible_neighbours.append(tour[:i]+ part2[::-1]+part1)
                        all_possible_neighbours.append(tour[:i]+ part2+part1)
                else:
                    if ((j-i)>=2 and (k-j)>=2 and (n-(k-i))>=2):
                        part1=tour[i:j]
                        part2=tour[j:k]
                        all_possible_neighbours.append(tour[:i]+part1[::-1]+ part2[::-1]+tour[k:])
                        all_possible_neighbours.append(tour[:i]+ part2+part1[::-1]+tour[k:])
                        all_possible_neighbours.append(tour[:i]+ part2[::-1]+part1+tour[k:])
                        all_possible_neighbours.append(tour[:i]+ part2+part1+tour[k:])
    if(n<=5):
        assert(len(all_possible_neighbours)==0)
    else:
        assert(len(all_possible_neighbours)==4*(n*(n-1)*(n-2)/6-n*(n-3)))
    "*** --------------  ***"
    return all_possible_neighbours    
...
def hillclimbUtil(tour, getNeighbours):
	...
	l, t = min((getTourLength(neighbour), neighbour) for neighbour in neighbours)
	return l, t
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    while True:
    	l, t = hillclimbUtil(minTour, getNeighbours)
    	if(l >= tourLengthList[-1]):##TODO: equality sign??
    		break
    	tourLengthList.append(l)
    	minTour = t
    tourLengthList = tourLengthList[1:]
    "*** --------------  ***"
    return tourLengthList, minTour
...
def nearestNeighbourTour(initial_city):
    ...
    "*** YOUR CODE HERE ***"
    assert(initial_city in nodeDict)
    rem = list(nodeDict.keys())
    tour.append(initial_city)
    rem.remove(initial_city)
    while(len(rem) > 0):
        d, n = min((getDistance(nodeDict[tour[-1]], nodeDict[n]), n) for n in rem)
        tour.append(n)
        rem.remove(n)
    "*** --------------  ***"
    return tour
...
def eucledianTour(initial_city):
    ...
    "*** YOUR CODE HERE ***"
    # part 1
    for i ,ni in nodeDict.items():
        for j, nj in nodeDict.items():
            if(i<j):
                edgeList.append([i,j,getDistance(ni, nj)])
    "*** --------------  ***"
    ...
    '''FINISHES HERE'''
    fin_ord = finalOrder(mst, initial_city, 0)
    return fin_ord
...
def finalOrder(mst, initial_city, parent):
    fin_order = []
    "*** YOUR CODE HERE ***"
    # for part 3
    fin_order.append(initial_city)
    for (i,j) in mst:
        if(i==initial_city and j!=parent):
            fin_order = fin_order + finalOrder(mst, j, initial_city)
        elif(j==initial_city and i!=parent):
            fin_order = fin_order + finalOrder(mst, i, initial_city)
    "*** --------------  ***"
    return fin_order
