...
class SudokuSearchProblem:
    ...
    def isGoalState(self, state):
        ...
        """
        return all([len(values)==1 for square, values in state.items()])
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        ## YOUR CODE HERE
        if not all(len(values) == 1 for square, values in state.items()):
            n,s = min((len(values), square) for square, values in state.items() if len(values) > 1)
            for d in state[square]:
                next_state = assign(state.copy(), square, d)
                if not next_state is False:
                    successors.append((next_state, (start, d), 1))
        return successors
